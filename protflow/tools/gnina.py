...
This module integrates **GNINA** (docking and CNN scoring) into the ProtFlow
framework. It automates receptor/ligand preparation, autobox definition,
job submission (local or HPC), and result collation into tidy tables ready
for downstream analysis.

Features
--------
- Prepares apo receptors and ligands, with optional chain-based autoboxing.
- Launches GNINA runs via a `JobStarter` backend and handles log/score outputs.
- Splits multimodel ligand PDBs into single-model poses and reattaches them to
  the apo receptor for convenient per-pose output files.
- Produces a consolidated pandas DataFrame with docking scores and file paths.

Typical usage
-------------
Create an instance and call :py:meth:`GNINA.run`:
...
.. code-block:: python
    ...
    poses = gnina.run(
        ...
        ligand_chain="Z",                 # or use ligand_path=...
        jobstarter=jobstarter,
...
Notes
-----
This runner will skip work if a prior scorefile exists (unless `overwrite=True`)
and will save a new scorefile once collection is complete, mirroring the ESMFold
runner’s behavior. :contentReference[oaicite:3]{index=3}
"""
# general imports
import os
import logging
...
class GNINA(Runner):
    ...
    The `GNINA` class provides the necessary methods to execute GNINA runs within the ProtFlow framework. 
    This class is responsible for managing the configuration, execution, and output processing of GNINA tasks.

    Detailed Description
    --------------------
    The `GNINA` class integrates GNINA into the ProtFlow pipeline by setting up the environment, 
    running the diffusion process, and collecting the results. It ensures that the inputs and outputs 
    are handled efficiently, making the data readily available for further analysis.
    ...
    Usage
    -----
    To use this class, initialize it with the appropriate script and Python paths, 
    along with an optional job starter. The main functionality is provided through the `run` method, 
    which requires parameters such as poses, prefix, and additional options for customization.
    ...
    Notes
    -----
    This class is designed to work within the ProtFlow framework and assumes that the necessary configurations 
    and dependencies are properly set up. It leverages shared data structures and configurations from ProtFlow 
    to provide a seamless integration experience.
    ...
    0.1.0
    """
    def __init__(self, python_path: str|None = None, script_path:str|None = None, pre_cmd:str=protflow.config.GNINA_PRE_CMD, jobstarter:JobStarter=None) -> None:
        """
        ...
        self.python_path = python_path or load_config_path(config, "GNINA_PYTHON_PATH")
        self.pre_cmd = pre_cmd or load_config_path(config, "GNINA_PRE_CMD", is_pre_cmd=True)
...
def collect_scores(work_dir:str, ligand_chain:str, ligand_name: str = None) -> pd.DataFrame:
    ...
    if ligand_name:
        ...
        resid = "LIG"  # Default residue name if not specified
    ...
    for ligand in ligands:
        ...
        for line in lines:
            if line.startswith("MODEL"):
                inside_model = True
                current_block = [line]
            elif inside_model and line.startswith("CONECT"):
                inside_model = False
                model_blocks.append(current_block)
            elif inside_model:
                current_block.append(line)

        if len(model_blocks) != len(matching_scores):
            logging.warning(f"Mismatch: {ligand} has {len(model_blocks)} models, but {len(matching_scores)} scores")

        for _, (model_lines, row) in enumerate(zip(model_blocks, matching_scores.itertuples())):
            pose_desc = row.poses_description
            model_lines[0] = f"{pose_desc}\n"  # Replace 'MODEL' line
            model_lines.append("TER\nEND\n")   # Add footer

            # Modify HETATM lines to change "UNL     1" to "LIG Z   1" --> Adds chain identification 
            for idx, line in enumerate(model_lines):
                if line.startswith("HETATM"):
                    model_lines[idx] = line.replace("UNL     1", f"{resid} {ligand_chain}   1")
    ...
    ligand_df = pd.DataFrame({"docked_ligand_path": ligand_paths,"poses_description": poses_descriptions})
    ...
    location = [add_ligand_to_pose(pose_path=row['input_path'], ligand_path=row['docked_ligand_path'], ligand_chain=ligand_chain, out_dir=output_dir) for _, row in scores.iterrows()]
